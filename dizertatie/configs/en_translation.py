...
def get_en_translation_config() -> Dict[str, List[ExperimentConfig]]:
    translators = ["mistral", "chatgpt3.5", "deepl", "gcloud"]
    r = {}

    for t in translators:
        r[t] = [
            __make_ro_sent_config(t),
            __make_rupert_config(t),
            __make_ro_text_summarization_config(t)
        ]
...
def __make_ro_text_summarization_config(translator: str) -> ExperimentConfig:
    return ExperimentConfig(
        ...
        translation_config=TranslationConfig(
            path=DATA_PATH.joinpath("translations", "ro_text_summarization_subsample_translations.json"),
...
def __make_rupert_config(translator: str) -> ExperimentConfig:
    return ExperimentConfig(
        dataset_name="Rupert",
        dataset_config=DatasetConfig(
        ...
        translation_config=TranslationConfig(
            path=DATA_PATH.joinpath("translations", "rupert_subsample_translations.json"),
            translator=translator
        )
    )
